...
namespace Lab5
{
    public partial class Form1a : SwitchableForm
    {
        ...
        public Form1a()
        {
            ...
            Resize += (s, e) => { if (_def != null && _lastExpanded != null) Render(); };
        }
    ...
    internal static class LSystemEngine
    {
        ...
        {
            public readonly Vec2 A, B; public readonly bool Draw;
            public Segment(in Vec2 a, in Vec2 b, bool draw) { A = a; B = b; Draw = draw; }
        }

        public static List<Segment> Trace(string sequence, double stepLen, double angleDeg, double initialDirDeg, double jitterDeg, Random rng)
        {
            ...
            foreach (char ch in sequence)
            {
                switch (ch)
                {
                    ...
                    {
                        var rad = Deg2Rad(dir);
                        var delta = new Vec2(Math.Cos(rad) * stepLen, Math.Sin(rad) * stepLen);
                        var next = new Vec2(pos.X + delta.X, pos.Y + delta.Y);
                        segs.Add(new Segment(pos, next, draw: true));
                        pos = next;
                        break;
                    }
                    case 'f': // шаг без рисования
                    {
                        var rad = Deg2Rad(dir);
                        var delta = new Vec2(Math.Cos(rad) * stepLen, Math.Sin(rad) * stepLen);
                        pos = new Vec2(pos.X + delta.X, pos.Y + delta.Y);
                        break;
                    }
                    case '+':
                    ...
                    case ']':
                        ...
                        break;
                    default:
                        // Прочие символы (X и т.п.) — просто маркеры, игнорируем на этапе рисования
                        break;
                }
            }
            return segs;
